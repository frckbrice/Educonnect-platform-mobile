...
export async function POST(request: Request) {
    try {
        ...
        if (contentType.includes("application/json")) {
            ...
            try {
                /**
                 * formData.get() does not exist. The FormData implementation 
                 * in React Native is write-only — it's designed for appending
                 *  data to send in a request
                 * 
                 * Solution: Use request.text() and Parse Manually.
                 */
                ...
                if (platform === "native" && params.get("refreshToken")) {
        ...
        if (platform === "web" && !refreshToken) {
            ...
            console.log("\n\n Using web platform with cookies for refresh token");
        ...
        if (!refreshToken) {
            ...
            console.log("\n\n No refresh token found, trying to use access token as fallback");
